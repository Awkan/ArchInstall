...
dialog --title "Welcome !" --msgbox "Welcome to Awkan dotfiles and software installation script for Arch linux. \n" 10 60

# Ask for some configuration parameters
...
# Chose which softwares ton install
...
options=(E "Essentials" on
         ...
         S "Ripgrep" on
         C "Network Configuration" on
         B "Browsers (firefox + chromium)" off
         P "Programming environments (PHP, Docker, ...)" on
         X "KeepassX" on
         J "Jrnl" on
         Y "Mysql tools" on
         D "Thunderbird" off
...
curl https://raw.githubusercontent.com/Awkan/ArchInstall/master/progs.csv > /tmp/progs.csv
